// Hybrid review for positions-only chunk extraction: deterministic pre-checks + LLM review.
...
import {
  ...
  buildDeterministicPositionsReviewReport,
...
import { reviewChunkPositions, saveArtifact } from "../../../lib/extraction-qa/openai-qa.ts";
...
import {
  asExtractionJson,
  clampInt,
  chunkPositionsReviewPasses,
  corsHeaders,
  isPositionsPipelineEmpty,
  json,
  resolvePositionsReviewFailureStatus,
...
export const handler = async (req: Request) => {
  ...
  if (chunks.length === 0) {
    return json({
      ok: true,
      processed: 0,
      message: "No chunks ready for positions review",
  ...
  if (!dryRun) {
    try {
      const { data: runData } = await supabase
        .from("pipeline_runs")
        .insert({
          pipeline_name: "validate_chunk_positions",
          status: "running",
  ...
  for (const chunk of chunks) {
    try {
      ...
      const extractionRaw = asPositionsExtractionJson(chunk.positions_extraction_json);
      const positionsWithIds = await ensureStablePositionIds(
        (Array.isArray(extractionRaw.positions) ? extractionRaw.positions : []) as Array<
          Record<string, unknown>
        >,
        chunk.story_id,
        chunk.chunk_index
      );
      const extraction = asExtractionJson({ positions: positionsWithIds });
      const backfilledPositionIds = (Array.isArray(extractionRaw.positions) ? extractionRaw.positions : []).some(
        (position) =>
          position == null ||
          typeof position !== "object" ||
          typeof (position as Record<string, unknown>).position_id !== "string"
      );
      const metadata = await loadStoryMetadata(supabase, chunk.story_id, chunk.chunk_index);

      const { data: chunkMeta } = await supabase
        .from("story_chunks")
        .select("positions_qa_validation_attempt_count, extraction_json")
        .eq("story_id", chunk.story_id)
        .eq("chunk_index", chunk.chunk_index)
        .single();

      const priorAttempts = chunkMeta?.positions_qa_validation_attempt_count ?? 0;
      const attemptNumber = priorAttempts + 1;
      const materialityWarnings = getMaterialityWarnings(sourceText, extraction, { positionsOnly: true });

      let reviewReport: PositionsReviewReport;

      if (isPositionsPipelineEmpty(extraction)) {
        ...
        if (auto.passes) {
          reviewReport = {
            ...
            passes_review: true,
            summary: "Empty positions extraction on trivial chunk — auto-passed.",
            deterministic_issues: auto.deterministic_issues,
          };
        } else {
          reviewReport = buildDeterministicPositionsReviewReport(
            {
              passes: false,
        ...
        if (!strictPre.passes) {
          reviewReport = buildDeterministicPositionsReviewReport(strictPre, attemptNumber);
        } else {
          const claimsBlob =
            chunkMeta?.extraction_json !== null &&
            typeof chunkMeta?.extraction_json === "object" &&
            !Array.isArray(chunkMeta?.extraction_json)
              ? (chunkMeta.extraction_json as { claims?: unknown[] }).claims
              : undefined;

          reviewReport = await reviewChunkPositions(
            OPENAI_API_KEY,
            MODEL,
            activePrompt.systemPrompt,
            {
              ...
              existing_claims: Array.isArray(claimsBlob) ? claimsBlob : [],
      ...
      if (chunkPositionsReviewPasses(reviewReport)) {
        finalStatus = "passed";
        validatedAt = now;
      } else {
      ...
      if (!dryRun) {
        const chunkUpdate: Record<string, unknown> = {
          positions_qa_status: finalStatus,
          positions_qa_review_report: reviewReport,
          positions_qa_validation_report: {
          ...
          positions_qa_validation_attempt_count: nextAttemptCount,
          positions_qa_validated_at: validatedAt,
        };
        if (backfilledPositionIds) {
          chunkUpdate.positions_extraction_json = extraction;
        }
