...
export const handler = async (req: Request) => {
  ...
  const knnPairs = await runCypher<{
    ...
    WITH p1, p2,
         reduce(dot = 0.0, i IN range(0, size(p1.embedding)-1) |
           dot + p1.embedding[i] * p2.embedding[i]) AS dot,
         sqrt(reduce(s = 0.0, x IN p1.embedding | s + x*x)) AS n1,
         sqrt(reduce(s = 0.0, x IN p2.embedding | s + x*x)) AS n2
    WITH p1, p2, CASE WHEN n1 > 0 AND n2 > 0 THEN dot / (n1 * n2) ELSE 0.0 END AS score
    WHERE score >= $minSim
    RETURN p1.uid AS a, p2.uid AS b,
           'embedding_knn' AS blockReason,
           'sim:' + left(p1.uid, 12) AS topicKey,
           score
    ORDER BY score DESC
    LIMIT $limit
    `,
