...
Upstream: graph-worker Arguments + gold-seeded `:Question` registry (`npx tsx scripts/seed-question-registry.ts`). Downstream: Explore/Admin via `graph_*` projections. Operators inspect mint quarantine on `/admin/graph-controversies` ([admin observability](../../../docs/admin-observability.md)).
