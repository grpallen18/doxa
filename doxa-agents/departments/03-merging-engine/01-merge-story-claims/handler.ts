// Merge parked chunk claims into story_claims (claims-only pipeline).
...
export const handler = async (req: Request) => {
  ...
  if (singleStoryId) {
    ...
    if (!readyIds.has(singleStoryId)) {
      ...
      return json(
        {
          error:
            "Story is not ready to merge — all chunks must pass QA with empty repair queue and no story claims yet.",
  ...
  for (const storyId of toProcess) {
    const { data: chunks } = await supabase
      .from("story_chunks")
      .select("extraction_json, claims_merge_eligibility")
      .eq("story_id", storyId)
      .order("chunk_index", { ascending: true });

    const blobs = (chunks ?? [])
      .map((c: { extraction_json: unknown; claims_merge_eligibility: unknown }) => {
        const mergeState = parseClaimsMergeEligibility(c.claims_merge_eligibility);
        const parked = assembleMergeClaims(mergeState);
        const source =
          parked.claims.length > 0 ? parked : c.extraction_json;
        return source;
      })
