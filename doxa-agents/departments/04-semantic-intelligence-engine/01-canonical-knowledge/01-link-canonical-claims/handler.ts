...
export const handler = async (req: Request) => {
  ...
  let storyClaims = (Array.isArray(rows) ? rows : []).filter(
    (r): r is { story_claim_id: string; story_id: string; raw_text: string } =>
      typeof r === "object" &&
      r !== null &&
      typeof (r as { story_claim_id: unknown }).story_claim_id === "string" &&
      typeof (r as { story_id: unknown }).story_id === "string"
  );

  if (singleStoryId) {
    const allowed = await isStoryQaPassed(supabase, singleStoryId);
    if (!allowed) storyClaims = [];
  } else if (!singleClaimId) {
    const passed = await loadQaPassedStoryIds(supabase);
    storyClaims = storyClaims.filter((r) => passed.has(r.story_id));
  } else {
    const storyId = storyClaims[0]?.story_id;
    if (storyId && !(await isStoryQaPassed(supabase, storyId))) storyClaims = [];
  }
