...
export const handler = async (req: Request) => {
  ...
  let fetchQuery = supabase
    ...
    .select("story_position_id, story_id, raw_text")
  ...
  let storyPositions = (Array.isArray(rows) ? rows : []).filter(
    (r): r is { story_position_id: string; story_id: string; raw_text: string } =>
      typeof r === "object" &&
      r !== null &&
      typeof (r as { story_position_id: unknown }).story_position_id === "string" &&
      typeof (r as { story_id: unknown }).story_id === "string"
  );

  if (singleStoryId) {
    const allowed = await isStoryQaPassed(supabase, singleStoryId);
    if (!allowed) storyPositions = [];
  } else if (!singlePositionId) {
    const passed = await loadQaPassedStoryIds(supabase);
    storyPositions = storyPositions.filter((r) => passed.has(r.story_id));
  } else {
    const storyId = storyPositions[0]?.story_id;
    if (storyId && !(await isStoryQaPassed(supabase, storyId))) storyPositions = [];
  }
