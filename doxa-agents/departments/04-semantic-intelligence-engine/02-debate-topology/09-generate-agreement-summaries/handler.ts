// Supabase Edge Function: generate_agreement_summaries.
...
const MAX_AGREEMENTS_TO_SCAN = 600;
...
async function generateSummaryBatch(
  ...
  const blocks = positionsTexts
    .map((texts, i) => `Agreement ${i + 1} claims:\n${texts.slice(0, 8).map((t, j) => `${j + 1}. ${t}`).join("\n")}`)
    .join("\n\n");

  const system = `Given multiple agreement sets below, produce a JSON array with one object per agreement: [{"label":"...","summary":"..."}, ...]
For each agreement: label = short 2-5 word stance name; summary = 2-5 sentences, neutral and factual.`;
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      return positionsTexts.map(() => ({ label: "Agreement", summary: "No summary." }));
    }
    return parsed.map((p, i) => ({
      label: ((parsed[i]?.label ?? p?.label ?? "Agreement") as string).slice(0, 100),
      summary: ((parsed[i]?.summary ?? p?.summary ?? "No summary.") as string).slice(0, 1000),
    }));
  } catch {
    return positionsTexts.map(() => ({ label: "Agreement", summary: content.slice(0, 1000) || "No summary." }));
  }
}
...
export const handler = async (req: Request) => {
  ...
  const { data: agreementRows } = await supabase
    ...
    .select("agreement_cluster_id, membership_fingerprint")
    .eq("status", "active")
    .limit(MAX_AGREEMENTS_TO_SCAN);

  const agreements = (agreementRows ?? []) as Array<{ agreement_cluster_id: string; membership_fingerprint: string | null }>;
  if (agreements.length === 0) {
    return json({ ok: true, llm_created: 0, sync_updated: 0, cache_misses: 0, cache_synced: 0, message: "No active agreement clusters" });
  }

  const cacheMisses: Array<{ aid: string; fingerprint: string }> = [];
  for (const { agreement_cluster_id: aid, membership_fingerprint: fp } of agreements) {
    if (cacheMisses.length >= LLM_BATCH_SIZE) break;
    if (!fp) continue;

    const { data: cached } = await supabase
      .from("agreement_summary_cache")
    ...
    cacheMisses.push({ aid, fingerprint: fp });
  }
  ...
  async function runLLM(): Promise<number> {
    ...
    const aids = cacheMisses.map((c) => c.aid);
    const { data: accRows } = await supabase
      .from("agreement_cluster_positions")
      .select("agreement_cluster_id, canonical_position_id, story_positions(story_position_claim_links(story_claims(claim_id)))")
      .in("agreement_cluster_id", aids)
      .eq("membership_kind", "core");

    const aidToClaimIds = new Map<string, string[]>();
    for (const row of accRows ?? []) {
      const aid = (row as { agreement_cluster_id: string }).agreement_cluster_id;
      const links = (row as {
        story_positions?: {
          story_position_claim_links?: Array<{ story_claims?: { claim_id?: string } | null }>;
        } | null;
      }).story_positions?.story_position_claim_links;
      const arr = aidToClaimIds.get(aid) ?? [];
      for (const link of links ?? []) {
        const cid = link.story_claims?.claim_id;
        if (cid && arr.length < 10 && !arr.includes(cid)) arr.push(cid);
      }
      aidToClaimIds.set(aid, arr);
    }

    const allClaimIds = Array.from(new Set([...aidToClaimIds.values()].flat()));
    ...
    const toProcess: Array<{ aid: string; fingerprint: string; texts: string[] }> = [];
    for (const { aid, fingerprint } of cacheMisses) {
      const cids = aidToClaimIds.get(aid) ?? [];
      const texts = cids.map((cid) => claimIdToText.get(cid)).filter(Boolean) as string[];
      if (texts.length > 0) {
        toProcess.push({ aid, fingerprint, texts });
      }
    }
    ...
    try {
      ...
      const aidsForCentroid = toProcess.map((t) => t.aid);
      const { data: centroidRows } = await supabase
        .from("agreement_clusters")
        .select("agreement_cluster_id, centroid_embedding")
        .in("agreement_cluster_id", aidsForCentroid);

      const centroidByAid = new Map<string, number[]>();
      for (const row of centroidRows ?? []) {
        const emb = parseEmbedding((row as { centroid_embedding?: unknown }).centroid_embedding);
        if (emb && emb.length > 0) {
          centroidByAid.set((row as { agreement_cluster_id: string }).agreement_cluster_id, emb);
        }
      }

      const labelSummaryTexts = results.map((r) => {
        const { label, summary } = r ?? { label: "Agreement", summary: "No summary." };
      ...
      for (let j = 0; j < toProcess.length; j++) {
        const { aid, fingerprint } = toProcess[j];
        const { label, summary } = results[j] ?? { label: "Agreement", summary: "No summary." };
        const centroid = centroidByAid.get(aid);
        const labelEmb = labelEmbeddings[j];

        if (!centroid || !labelEmb || labelEmb.length !== centroid.length) continue;
        const sim = cosineSimilarity(labelEmb, centroid);
        if (sim < driftThreshold) continue;

        updates.push(
          supabase.from("agreement_clusters").update({ label, summary }).eq("agreement_cluster_id", aid),
          supabase.from("agreement_summary_cache").upsert(
      ...
      console.error("[generate_agreement_summaries] LLM batch:", e);
      return 0;
    }
  }

  async function runSync(): Promise<number> {
    const { data, error } = await supabase.rpc("sync_agreement_summaries_from_cache", { p_max_count: SYNC_BATCH_SIZE });
    if (error) {
      console.error("[generate_agreement_summaries] sync RPC:", error.message);
      return 0;
    }
  ...
  return json({
    ...
    console.error("[generate_agreement_summaries]", msg);
