...
import { getPropositionHubGraph } from '@/lib/neo4j/queries/hub'

type RouteParams = { params: Promise<{ uid: string }> }

/** Cross-story Proposition hub subgraph from Neo4j. Admin only. */
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const graph = await getPropositionHubGraph(uid)
    if (!graph) {
      return NextResponse.json(
        {
          data: null,
          error: { message: 'No Proposition found in Neo4j for this uid' },
        },
        { status: 404 }
      )
    }
