...
import { getControversyHubGraph } from '@/lib/neo4j/queries/hub'

type RouteParams = { params: Promise<{ uid: string }> }

/** Cross-story Controversy hub subgraph from Neo4j. Admin only. */
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const graph = await getControversyHubGraph(uid)
    if (!graph) {
      return NextResponse.json(
        { data: null, error: { message: 'No Controversy found in Neo4j for this uid' } },
        { status: 404 }
      )
    }
