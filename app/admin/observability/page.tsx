...
function ScrapeAreaChart({
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    fetch(`/api/admin/observability/scrape-stats?range=${timeRange}`)
...
function ScrapeDrillDownPanel({
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/observability/scrape-drilldown?bucket=${encodeURIComponent(selectedBucket)}&granularity=${granularity}&outcome=${outcome}`)
      .then((res) => (res.ok ? res.json() : null))
      .then((json) => {
        if (cancelled || !json) return
...
function DomainScrapeRateList() {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    fetch('/api/admin/observability/scrape-stats-by-source')
      .then((res) => (res.ok ? res.json() : null))
      .then((json) => {
        if (cancelled || !json) return
