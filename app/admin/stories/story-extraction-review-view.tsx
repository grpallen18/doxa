...
export function StoryExtractionReviewView({
  ...
  const spanHighlight = useMemo(
    () => ({
      articleText: story.article_text,
      chunks: payload.chunks,
      onHighlightSpan: setHighlightSpan,
    }),
    [story.article_text, payload.chunks]
  )

  const clearHighlight = useCallback(() => setHighlightSpan(null), [])
  ...
  return (
    <div className="grid min-h-[400px] grid-cols-1 gap-4 lg:h-[calc(100vh-8rem)] lg:min-h-0 lg:grid-cols-2 lg:items-stretch">
      <Panel
        variant="soft"
        interactive={false}
