-- Stack-style QA revert: undo one review or refine pass at a time (claims + positions lanes).
-- Refine revert restores artifact input_snapshot; review revert pops the latest review run batch.

CREATE OR REPLACE FUNCTION public.revert_qa_status_from_review_report(
  p_report jsonb,
...
AS $$
  SELECT CASE
    WHEN COALESCE((p_report->>'passes_review')::boolean, false) = true
      AND COALESCE(p_report->>'recommended_action', '') = 'validate'
    THEN 'passed'
    WHEN COALESCE(p_attempt_number, 0) >= 3 THEN 'needs_human_review'
    WHEN COALESCE(p_report->>'recommended_action', '') = 'needs_refinement' THEN 'needs_refinement'
