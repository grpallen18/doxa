-- Include chunk scope in story_history meta when mirroring story_step_runs (chunk agent flow audit).
...
BEGIN
  ...
  v_chunk_indices := COALESCE(p_meta->'chunk_indices', '[]'::jsonb);
  IF p_chunk_index IS NOT NULL
    AND NOT v_chunk_indices @> jsonb_build_array(p_chunk_index)
  THEN
    v_chunk_indices := v_chunk_indices || jsonb_build_array(p_chunk_index);
  END IF;
...
COMMENT ON FUNCTION public.append_story_step_run IS
  'Append one story-scoped pipeline step run row and mirror step labels + chunk scope to story_history.';
