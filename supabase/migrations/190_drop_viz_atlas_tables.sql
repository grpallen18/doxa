-- Drop Atlas visualization tables (feature removed).

DROP TABLE IF EXISTS public.viz_edges CASCADE;
DROP TABLE IF EXISTS public.viz_nodes CASCADE;
DROP TABLE IF EXISTS public.viz_maps CASCADE;

-- Stop legacy atlas cron if it was scheduled on this database.
DO $$
BEGIN
  PERFORM cron.unschedule('generate-atlas-map-weekly');
EXCEPTION
  WHEN OTHERS THEN
    NULL;
END $$;
...
-- Refresh purge_engine_data() so it no longer references dropped viz_* tables.
-- Source: supabase/routines/purge_engine_data.sql (regenerated via agents:refresh).
