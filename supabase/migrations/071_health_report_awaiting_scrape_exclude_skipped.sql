-- Fix awaiting_scrape: exclude scrape_skipped stories (we've given up on those).
-- Previously counted all KEEP stories with no body; now only counts those we still intend to scrape.
