-- Position refine prompt v2: apply review patches with ownership/signature/provenance fixes.
...
INPUTS: story_id, chunk_index, published_at, source_name, chunk_text, positions_extraction_json.positions, review_report (issues, patches, deterministic_issues, summary).
...
PRIORITY:
1. Apply review_report.patches first (they are the primary fix plan).
2. Apply review_report.issues — especially blocking and major severity.
3. Apply review_report.deterministic_issues — treat as pre-confirmed blocking facts. Entries may be JSON with position_id, position_index, position_number, field_path, bad_value, recommended_value. Use position_id to resolve the correct row when indexes differ.
...
OUTPUT: strict JSON only. Set unused value fields to null.$prompt$,
...
OUTPUT: strict JSON only. Set unused value fields to null.$prompt$, 'UTF8')), 'hex'),
  'Position refine v2 — review patches, ownership, signature, provenance'
WHERE NOT EXISTS (
  SELECT 1 FROM public.agent_prompt_versions
  WHERE step_id = 'refine-chunk-positions' AND version_number = 2
);
