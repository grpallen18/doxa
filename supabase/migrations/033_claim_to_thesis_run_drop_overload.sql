-- Remove one-argument overload so claim_to_thesis_run(5) resolves to the two-arg version (dry_run default false).

drop function if exists public.claim_to_thesis_run(int);
