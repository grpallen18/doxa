-- Add scrape_skipped_at: when scrape_skipped was set to true.
-- Enables 24h scrape-failure metrics (e.g. Discord health report).

alter table public.stories
  add column if not exists scrape_skipped_at timestamptz;

comment on column public.stories.scrape_skipped_at is 'When scrape_skipped was set to true (after 3 failures or Worker immediate failure). Null if not skipped.';

-- Update increment_scrape_fail_and_maybe_skip to set scrape_skipped_at when skipping
...
as $$
  ...
  set
    ...
    scrape_skipped_at = case when (scrape_fail_count + 1 >= 3) then now() else null end,
...
-- Update health report: scrape_failed = stories skipped in last 24h (was all-time snapshot)
