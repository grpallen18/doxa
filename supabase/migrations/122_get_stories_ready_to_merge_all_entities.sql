-- merge_story_entities (step: merge-story-entities): story_ids ready to merge when all chunks
-- are extracted and no story-level entity rows exist yet.
...
comment on function public.get_stories_ready_to_merge(int) is 'Returns story_ids ready for merge_story_entities (all chunks extracted, no story-level entities yet). Ordered by created_at asc.';
