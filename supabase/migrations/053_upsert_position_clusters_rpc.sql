-- RPC: upsert_position_clusters_batch
-- Receives clusters from build_position_clusters; upserts by fingerprint, syncs claims,
-- records lineage for merge/split, marks orphans inactive. Single transaction.
...
begin
  ...
  loop
    ...
    -- Find new clusters that contain any of orphan's claims
    select array_agg(distinct c->>'fingerprint')
    into overlapping_fps
    from jsonb_array_elements(p_clusters) as c,
         jsonb_array_elements_text(c->'claim_ids') as cid,
