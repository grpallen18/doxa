-- Pipeline step history: record previous → current step labels in story_history meta.
...
BEGIN
  ...
  IF v_processed IS NOT NULL THEN
    v_current_value := v_step_label || ' · ' || v_processed::text || ' processed';
  ELSE
    v_current_value := v_step_label || ' · ' || v_outcome_label;
  END IF;
