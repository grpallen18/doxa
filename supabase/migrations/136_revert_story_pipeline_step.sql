-- Admin step revert through extract primary claims (ingestion + chunk + extract).
-- Strict tip-only revert is enforced in the admin UI; RPC validates step was executed.

alter table public.stories
  add column if not exists pending_review_ran_at timestamptz;

comment on column public.stories.pending_review_ran_at is
  'Set when review_pending_stories resolves a PENDING story; cleared on revert.';
  ...
  if exists (select 1 from public.story_claims sc where sc.story_id = p_story_id)
    ...
    or exists (
      ...
      where sc.story_id = p_story_id
        and sc.extraction_qa_status is not null
