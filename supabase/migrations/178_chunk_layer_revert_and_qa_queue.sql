-- Chunk-layer revert (single chunk) and claims refine queue parity with positions.

set search_path = public, extensions;

-- Claims refine queue: include refinable needs_human_review (parity with positions).
  ...
  where sc.extraction_json is not null
    and (
      ...
      or (
        ...
        and (
          sc.extraction_qa_status = 'needs_refinement'
          or (
            sc.extraction_qa_status = 'needs_human_review'
            and (
              sc.extraction_qa_review_report->>'recommended_action' = 'needs_refinement'
              or jsonb_array_length(coalesce(sc.extraction_qa_review_report->'patches', '[]'::jsonb)) > 0
              or exists (
                select 1
                from jsonb_array_elements(coalesce(sc.extraction_qa_review_report->'issues', '[]'::jsonb)) issue
...
-- Revert one chunk-layer step for a single chunk (admin chunk agent flow).
