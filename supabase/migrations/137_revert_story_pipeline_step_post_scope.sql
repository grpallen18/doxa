-- Fix revert_story_pipeline_step post-scope guard: legacy backfill sets chunk QA to passed
-- without running validate-chunk-claims; do not treat that alone as blocking revert.
...
begin
  ...
  if exists (select 1 from public.story_claims sc where sc.story_id = p_story_id)
    ...
    or exists (
      ...
      where sc.story_id = p_story_id
        and (
          sc.extraction_qa_validated_at is not null
          or sc.extraction_qa_standardization_report is not null
          or (
            sc.extraction_qa_status is not null
            and sc.extraction_qa_status not in ('pending', 'passed')
          )
        )
    )
    or exists (
      select 1 from public.story_extraction_qa_artifacts a
