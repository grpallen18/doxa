-- Claims QA stack revert: include legacy artifact stages (chunk_review, chunk_refine, chunk_validate).
...
DECLARE
  ...
  v_claims_review_stages text[] := array[
    'chunk_review_claims',
    'chunk_review',
    'chunk_validate'
  ];
  v_claims_refine_stages text[] := array[
    'chunk_refine_claims',
    'chunk_refine'
  ];
  ...
  CASE p_step_id
    ...
    WHEN 'refine-chunk-claims' THEN
      ...
      WHERE a.story_id = p_story_id
        AND a.stage = ANY (v_claims_refine_stages)
      ...
      DELETE FROM public.story_extraction_qa_artifacts
      WHERE story_id = p_story_id
        AND stage = ANY (v_claims_refine_stages)
        AND (
    ...
    WHEN 'validate-chunk-claims' THEN
      ...
      FROM public.story_extraction_qa_artifacts a
      WHERE a.story_id = p_story_id
        AND a.stage = ANY (v_claims_review_stages)
      ORDER BY a.created_at DESC
      LIMIT 1;
      ...
      IF v_latest_run_at IS NULL THEN
        ...
        DELETE FROM public.story_extraction_qa_artifacts
        WHERE story_id = p_story_id
          AND stage = ANY (array_cat(v_claims_review_stages, v_claims_refine_stages));
      ELSE
        FOR v_rec IN
          SELECT a.id, a.chunk_index, a.input_snapshot, a.report, a.created_at, a.stage
          FROM public.story_extraction_qa_artifacts a
          WHERE a.story_id = p_story_id
            AND a.stage = ANY (v_claims_review_stages)
            AND (
        ...
        DELETE FROM public.story_extraction_qa_artifacts
        WHERE story_id = p_story_id
          AND stage = ANY (v_claims_review_stages)
          AND (
            (v_latest_run_id IS NOT NULL AND run_id = v_latest_run_id)
            OR (v_latest_run_id IS NULL AND created_at = v_latest_run_at)
          );
