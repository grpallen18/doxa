-- Chunk extraction QA: standardize step, validation attempt loop, refined queue caps.

set search_path = public, extensions;

alter table public.story_chunks
  add column if not exists extraction_qa_standardization_report jsonb,
  add column if not exists extraction_qa_validation_attempt_count int not null default 0;

comment on column public.story_chunks.extraction_qa_standardization_report is
  'Standardizer output: kept/merged/reclassified/discarded audit trail.';

comment on column public.story_chunks.extraction_qa_validation_attempt_count is
  'Failed validation attempts in current extract cycle (max 3 before needs_human_review).';
...
alter table public.story_chunks
  add constraint story_chunks_extraction_qa_status_check
    check (extraction_qa_status in (
      'pending', 'reviewed', 'standardized', 'needs_refinement', 'refined',
      'atoms_passed', 'passed', 'needs_human_review'
    ));

update public.story_chunks
set extraction_qa_status = 'standardized'
where extraction_qa_status = 'reviewed'
  and extraction_qa_validated_at is null;

comment on column public.story_chunks.extraction_qa_status is
  'Chunk QA: pending -> standardized -> atoms_passed (validate) -> passed (link). Refine loop: needs_refinement -> refined -> validate.';
...
comment on function public.get_chunks_ready_for_chunk_qa(text, int) is
  'Queue for chunk extraction QA: standardize | refine | validate | link.';
